...
void main() {
  ...
  group("product detail bloc", ()
  {
    ...
    group("ProductDetailBloc GetProductByID", () {
      blocTest<ProductDetailBloc, ProductDetailState>("get product detail by id[POST Method] case success case",
          setUp: () {
            ...
            when((){
              return utilityRepository.getByURL("$baseUrl$inventoryApiPath$path", {"pid": "test12345"}, headers: {"Authorization": "Bearer ","source": "LINE"});
            }).thenAnswer(
                  (_) async {
      ...
      //     act: (bloc) => bloc.add(const SetContactDetail(type: ContactType.maps, lat: 0, long: 90, keyword: "test")),
      //     expect: () => <ProductDetailState>[
      //       const ProductDetailState(type: ContactType.maps, lat: 0, long: 90, keyword: "test")
      //     ]);
      //
      // blocTest<ProductDetailBloc, ProductDetailState>("set contact detail type none",
      //     setUp: () {
      //     },
      //     build: () => ProductDetailBloc(),
      //     act: (bloc) => bloc.add(const SetContactDetail(type: ContactType.none)),
      //     expect: () => <ProductDetailState>[
      //       const ProductDetailState()
      //     ]);
    });

    group("ProductDetailBloc SetProduct", () {
